...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        #shortcut = self.shortcut(x) if self.stride == 1 and self.shortcut is not None else out # works with shortcut=nn.Sequential()
        shortcut = out
        if self.stride == 1:
            shortcut = x
...
def efficientnet_cifar(pretrained=None):
